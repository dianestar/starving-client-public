  ...
  return (
    <div className="w-[250px] h-[375px] bg-white border-solid border-2 border-neutral-200 flex flex-col items-center justify-between relative my-2">
      ...
      <section className="cursor-pointer flex flex-col items-center">
        <img
          className="absolute top-[50%] w-[60px] h-[60px] rounded-full object-cover"
          src={avatarImage || "/defaultAvatarImage.png"}
        />
        <span className="absolute top-[67%] text-neutral-400">{nickname}</span>
      </section>
      <section className="text-center cursor-pointer" onClick={pushToDetail}>
      ...
      <section className="w-full grid grid-cols-2 border-solid border-t-2 border-neutral-200 text-neutral-400 text-sm">
        <article
          className="w-full flex items-center justify-center border-solid border-r-2 border-neutral-200 cursor-pointer"
          onClick={onClickLike}
        >
          {liked ? (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-6 w-6 mr-1 text-pink-600"
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
            ...
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-6 w-6 mr-1 text-pink-300 hover:text-pink-600"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
